...
import {
   IconBrain, IconAlertTriangle, IconRefresh, IconRobot
               ...
               <div className="flex flex-col gap-4 overflow-y-auto max-h-[460px] pr-1 custom-scrollbar">
                  {data?.interventions?.map((item: any, idx: number) => (
                     <Card
                        ...
                        className={`rounded-[1.5rem] border-none shadow-lg cursor-pointer transition-transform hover:scale-[1.02] active:scale-95 ${item.priority === 'CRITICAL' ? 'bg-amber-600 text-white' : 'bg-emerald-950 text-white'}`}
                     >
                        <div className="p-4 flex flex-col gap-2">
                           ...
                           <div className="mt-2 flex items-center justify-end">
                              <span className="text-[8px] font-black uppercase tracking-widest bg-white/10 px-2 py-1 rounded-full">Take Action →</span>
                           </div>
